...
interface ShowroomWithDistance extends ShowroomRaw {
...
export function ShowroomsPage(_: ShowroomsPageProps) {
  ...
  useEffect(() => {
    ...
    const resolveList = (data: any, lang: "ar" | "en") => {
      if (Array.isArray(data)) {
        ...
        const fallbackList = data.find((item) => item && Array.isArray(item?.ar));
        if (fallbackList) return fallbackList.ar;
        if (data.length > 0 && Array.isArray(data[0]?.[lang])) return data[0][lang];
        if (data.length > 0 && Array.isArray(data[0]?.ar)) return data[0].ar;
        return data;
      }
      if (data && Array.isArray(data[lang])) return data[lang];
      if (data && Array.isArray(data.ar)) return data.ar;
      return [];
    };
    ...
    const normalizeData = (rawData: any) => {
      ...
      return resolveList(rawData, language)
        .map((item: any) => normalizeFromDb(item))
        .filter((s: ShowroomRaw) => s.isActive !== false && s.Loc_code && s.Latitude && s.Longitude);
    };
    ...
    (async () => {
      const loaded = await loadShowrooms();
      if (mounted) {
        setShowroomsData(loaded);
        setIsLoading(false);
      }
    })();
  ...
  // Calculate showrooms with distances (without sorting yet)
  const showroomsWithDistance = useMemo(() => {
    ...
    return showroomsData.map((showroom) => {
      ...
      const distanceKm = position && Number.isFinite(latitude) && Number.isFinite(longitude)
        ? calculateDistanceKm(position, { lat: latitude, lng: longitude })
        : null;
      return { ...showroom, distanceKm };
    });
  }, [currentUserPosition, showroomsData]);

  // Sort showrooms if we have a position
  const sortedShowroomsByDistance = useMemo(() => {
    if (currentUserPosition || getCachedPosition()?.position) {
      return [...showroomsWithDistance].sort((a, b) => {
  ...
  }, [showroomsWithDistance, currentUserPosition]);

  // Group by city and sort showrooms within each city
  const showroomsByCity = useMemo(() => {
    const grouped = sortedShowroomsByDistance.reduce((acc, showroom) => {
    ...
    // Sort showrooms within each city by distance
    Object.keys(grouped).forEach(city => {
      grouped[city] = grouped[city].sort((a, b) => {
        const distA = a.distanceKm ?? Number.MAX_SAFE_INTEGER;
        const distB = b.distanceKm ?? Number.MAX_SAFE_INTEGER;
        return distA - distB;
      });
    ...
    return grouped;
  }, [sortedShowroomsByDistance, isRTL]);

  // Sort cities based on the nearest showroom in each city
  const sortedCities = useMemo(() => {
    return Object.keys(showroomsByCity).sort((a, b) => {
  ...
  // Find the globally nearest showroom
  const nearestShowroom = sortedShowroomsByDistance
  ...
  useEffect(() => {
    // Only request location if we have showrooms loaded and no position yet
    if (!isLoading && showroomsData.length > 0 && !currentUserPosition && !getCachedPosition()?.position) {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            const pos = {
  ...
  }, [isLoading, showroomsData.length, currentUserPosition]);

  // Debug logging to help identify issues
  useEffect(() => {
    if (!isLoading) {
      console.log("Showrooms loaded:", showroomsData.length);
      console.log("Showrooms with distance:", showroomsWithDistance.length);
      console.log("Cities found:", Object.keys(showroomsByCity).length);
      console.log("Has user position:", !!currentUserPosition);
    }
  }, [isLoading, showroomsData.length, showroomsWithDistance.length, showroomsByCity, currentUserPosition]);
    ...
    <div dir={isRTL ? "rtl" : "ltr"} className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="mb-8 rounded-2xl border border-[#009FE3]/15 bg-white p-5 shadow-sm">
          ...
          {nearestShowroom && currentUserPosition && (
        ...
        {!isLoading && sortedCities.length === 0 && showroomsData.length === 0 && (
          <div className="text-center py-20 text-gray-500">
            {language === "ar" ? "لا توجد صالات عرض متاحة حالياً" : "No showrooms available right now"}
          </div>
        )}

        {!isLoading && showroomsData.length > 0 && sortedCities.length === 0 && (
          <div className="text-center py-20 text-gray-500">
            {language === "ar" ? "جاري تجهيز البيانات..." : "Preparing data..."}
          </div>
        )}
                  ...
                  return (
                    <div
                      ...
                      className={`bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 border ${
                        nearestShowroomCode === showroom.Loc_code && currentUserPosition
                      ...
                      >
                        ...
                        <div className="absolute top-4 left-4 right-4">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {typeof showroom.distanceKm === 'number' && currentUserPosition && nearestShowroomCode !== showroom.Loc_code && (
